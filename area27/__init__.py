...
def count_bam(bedfile, bamfile, nthreads=4, window=2000):
    ...
    list 
        List with counts.
    """
    f = partial(load_heatmap_data, datafile=bamfile, bins=1,  up=window//2, down=window//2, 
            rmdup=True, rpkm=False, rmrepeats=True,fragmentsize=None, dynam=False, guard=None)
    ...
    for i in range(0, total, batch_size):
        ...
        df.iloc[i:i + batch_size].to_csv(tmp.name, sep="\t", index=False, header=False)
...
def create_link_file(meanstd_file, genes_file, genome="hg38"):
    ...
    overlap["loc"] = overlap["chrom"] + ":" + overlap["start"].astype(str) + "-" + overlap["end"].astype(str)
    overlap["pos"] = ((overlap["start"] + overlap["end"]) / 2).astype(int)
    overlap = overlap[["gene", "loc", "pos"]]
    return overlap

def link_it_up(outfile, signal, meanstd_file=None, genes_file=None, names_file=None, threshold=2):
    if meanstd_file is None:
        meanstd_file = resource_filename(__name__, "data/remap.hg38.meanstd.tsv.gz")
    if genes_file is None:
        genes_file = resource_filename(__name__, "data/gencode.v30.TSS.all_transcripts.merged1kb.bed.gz")
    ...
    link_file = os.path.join(
            CACHE_DIR, 
            ".".join((
                splitextgz(meanstd_file),
                splitextgz(genes_file), 
                "feather"
                )))

    if not os.path.exists(link_file):
        sys.stdout.write("Creating link file\n")
        link = create_link_file(meanstd_file, genes_file)
    ...
    genes = pd.read_csv(genes_file,
                   sep="\t", index_col=2,
                   usecols=[0,1,3,5], names=[ "chrom","start","gene","strand"])
 
    link = link.join(signal[signal["signal"] > 2][["signal"]], on="loc").dropna()
    link = link.dropna()
...
def generate_signal(bam_file, window, meanstd_file=None, target_file=None, nthreads=4):
    ...
    with NamedTemporaryFile(prefix=f"area27.", suffix=".bed") as f:
        ...
        meanstd["index"].str.split("[:-]", expand=True).to_csv(f.name, sep="\t", index=False, header=False)
